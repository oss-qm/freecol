...
public class Colony extends Settlement implements Nameable, TradeLocation {
    ...
    private final int getNetProdOfQueue(GoodsType gt, BuildQueue q) {
...
loop:   for (WorkLocation wl : getWorkLocationsForProducing(goodsType)) {
    ...
    public Collection<StringTemplate> getProductionWarnings(GoodsType goodsType) {
        ...
        for (WorkLocation wl : getWorkLocationsForConsuming(goodsType)) {
            ProductionInfo pi = getProductionInfo(wl);
            if (pi != null)
                for (AbstractGoods ag : wl.getOutputList())
                    if (ag.isStorable())
     ...
     */
    @Override
    public void fillDisposables(List<FreeColGameObject> list) {
        synchronized (this.colonyTiles) {
            for (ColonyTile walk : this.colonyTiles)
                walk.fillDisposables(list);
        }
        synchronized (this.buildingMap) {
            for (Building walk : this.buildingMap.values())
                walk.fillDisposables(list);
        }
        super.fillDisposables(list);
    }
